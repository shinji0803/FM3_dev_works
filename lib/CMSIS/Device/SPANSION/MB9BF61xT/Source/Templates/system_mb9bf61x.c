...
void SystemCoreClockUpdate (void) {
  ...
  //   "Unordered accesses to a volatile location"
	/* masterClk = (Input Freq.)/K * N (M=don't care)　　　　　　　　　　　　　 */
...
/* Nemui Added */
#if (__HCLK > 72000000UL)
  FM3_FLASH_IF->FBFCR = 0x01;						       /* Enable Trace Buffer */ 
  do                                              
  {                                               
    u32IoRegisterRead = (FM3_FLASH_IF->FBFCR & 0x02);      /* Wait until Stable */ 
  }while ((FM3_CRG->SCM_STR & 0x02) != u32IoRegisterRead);
#endif
/* Nemui Added */
